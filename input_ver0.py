...
sys_id_dic = []
frame_id_dic = []
Foo_p = namedtuple("Foo_p", "Team, Player, x, y, v")
...
#------------------------------------------------------------------------------------
filename = "udp.out"
D = defaultdict(lambda: defaultdict(int)) #選手の位置情報 D[frame_id][sys_id]
...
for row in fin:
    ...
    for i in range(len(temp1) - 1):
        ...
        f = Foo_p(Team_id, Player_id, float(temp2[3]), float(temp2[4]), float(temp2[5]))
        D[frame_id][sys_id] = f
